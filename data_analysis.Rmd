...
```{r, eval = FALSE}
geocoding = read_excel("results/Geocoding ALL.xlsx") %>% 
  janitor::clean_names() %>% 
  select(-name, -note) %>% 
...
gis_csv = left_join(max_value, select(geo_new, project_id, zip, latitude, longitude, address = address_4), by = "project_id")
...
gis_zipcode = gis_csv %>% 
  ...
  summarise(
    ...
    trichloroethene_median = median(trichloroethene_max, na.rm = T),
    tetrachloroethene_median = median(tetrachloroethene_max, na.rm = T),
    btex_median = median(btex_max, na.rm = T)
  )

df = gis_csv %>% 
  summarise(
    tce_quantile = quantile(trichloroethene_max, probs = c(.1, .9), na.rm = T),
    pce_quantile = quantile(tetrachloroethene_max, probs = c(.1, .9), na.rm = T),
    btex_quantile = quantile(btex_max, probs = c(.1, .9), na.rm = T)
  ) %>% 
  mutate(
    quantile = c("10%", "90%")
  ) %>% 
...
# write_csv(df, "results/chem_quantile.csv")
```
...
percentage_plot = binary %>% 
  ...
  mutate(
    ...
    indicator = fct_relevel(indicator, ">= Health-based Concentrations", "< Health-based Concentrations")
  ) %>%
  filter(chem != "BTEX") %>% 
